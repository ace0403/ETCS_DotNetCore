...
$(function () {
    ...
    $(document).on('click', '#div-meallist .meal-item', async function () {
        var $item = $(this);
        ...
        if (isAllergenItem) {
            ...
            var consent = await showAllergenConsent(childName, allergies, 'meal');
    ...
    $(document).on('click', '#btnPlaceOrder', async function () {
        if (selectedIds.length === 0) {
            await showStyledAlert('Please select at least one item.');
            return;
        }
    ...
    $(document).on('click', '#btnConfirmOrder', function () {
        ...
        if (selectedIds.length === 0) {
            showStyledAlert('Please select at least one item.');
            return;
        }
...
function applyMenuFilters() {
    ...
    if ($empty.length) {
        if (visibleCount === 0 && $('#div-meallist .meal-item').length > 0) {
...
function getOrderSummary(mealItemList, preferredMealDate) {
    $.ajax({
...
function removeSummarySelection($btn) {
    ...
    selectedIds = selectedIds.filter(function (x) {
        ...
        if (!selectionId && parseInt(x.ItemId, 10) === itemId && String(x.MealDate) === mealDate) {
            return false;
        }
        return true;
    });
